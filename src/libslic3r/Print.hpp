...
public:
    ...
    static bool     update_layer_height_profile(const ModelObject &model_object, const SlicingParameters &slicing_parameters, std::vector<coordf_t> &layer_height_profile, const PrintObject *print_object = nullptr);
...
struct WipeTowerData
{
    ...
    float                                                 depth;
    std::vector<std::pair<float, float>>                  z_and_depth_pairs;
    // Per-layer reserve boxes for Local-Z unplanned toolchanges.
...
private:
    ...
    // Virtual mixed-filament manager. Populated in Print::apply() whenever
    // filament colours or mixed_filament_definitions change.
