...
// Only add a newline in case the current G-code does not end with a newline.
                                                  ...
                                                  bool local_z_unplanned, double local_z_nominal_layer_z)
    {
        ...
        auto emit_local_z_unplanned_toolchange = [&]() -> std::string {
            ...
            local_gcode += gcodegen.writer().unlift();
            ...
            if (gcodegen.writer().extruder() != nullptr) {
                ...
                if (gcodegen.m_config.z_hop_when_prime.get_at(gcodegen.m_writer.extruder()->id()))
                    local_gcode += gcodegen.retract(false, false, gcodegen.to_lift_type(type));
            }

            gcodegen.m_avoid_crossing_perimeters.use_external_mp_once();
            local_gcode += gcodegen.travel_to(wipe_tower_point_to_object_point(gcodegen, start_machine), erMixed,
                                        "Travel to Local-Z wipe tower reserve");
            local_gcode += gcodegen.unretract();

            if (!is_approx(tower_z, current_z)) {
                local_gcode += gcodegen.writer().retract();
                local_gcode += gcodegen.writer().travel_to_z(tower_z, "Travel to Local-Z tower layer");
                local_gcode += gcodegen.writer().unretract();
            }

            local_gcode += gcodegen.set_extruder(unsigned(extruder_id), toolchange_print_z);
            local_gcode += gcodegen.writer().travel_to_z(tower_z, "Force restore Local-Z tower Z", true);
            {
            ...
            local_gcode += gcodegen.unretract();
            local_gcode += gcodegen.writer().travel_to_xy(start_machine.cast<double>(), "Return to Local-Z wipe tower reserve");

            local_gcode += ";" + GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Height) + float_to_string_decimal_point(layer_height, 3) + "\n";
            local_gcode += ";" + GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Role) + ExtrusionEntity::role_to_string(erWipeTower) + "\n";
            local_gcode += ";" + GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Width) + float_to_string_decimal_point(line_width, 3) + "\n";
            ...
            local_gcode += gcodegen.writer().set_speed(feedrate, "Local-Z wipe tower reserve");
            ...
            for (size_t point_idx = 1; point_idx < local_path.size(); ++point_idx) {
                ...
                if (length <= EPSILON)
                    continue;
                local_gcode += gcodegen.writer().extrude_to_xy(machine_pt.cast<double>(), flow_per_mm * float(length),
            ...
            if (!is_approx(tower_z, current_z)) {
                ...
                if (can_wipe) {
                    ...
                    local_gcode += gcodegen.writer().retract(true, wipe_retractions.retractLengthBeforeWipe);
                    local_gcode += gcodegen.m_wipe.wipe(gcodegen, wipe_retractions.retractLengthDuringWipe, false, false);
                }
                local_gcode += gcodegen.writer().retract();
                gcodegen.m_wipe.reset_path();
                local_gcode += gcodegen.writer().travel_to_z(current_z, "Travel back to Local-Z pass");
                local_gcode += gcodegen.writer().unretract();
            }
        ...
        if (print.config().print_sequence == PrintSequence::ByObject && !has_wipe_tower) {
            ...
            if (has_wipe_tower && ! layers_to_print.empty()) {
                m_wipe_tower.reset(new WipeTowerIntegration(print.config(), print.get_plate_index(), print.get_plate_origin(), *print.wipe_tower_data().priming.get(),
                                                            print.wipe_tower_data().tool_changes, print.wipe_tower_data().local_z_reserve_boxes,
    ...
    const bool                               prime_extruder)
{
            ...
            for (size_t region_id = 0; region_id < layer.regions().size(); ++ region_id) {
                ...
                for (const ObjectByExtruder::Island::Region::Type entity_type : { ObjectByExtruder::Island::Region::INFILL, ObjectByExtruder::Island::Region::PERIMETERS }) {
                    for (const ExtrusionEntity *ee : (entity_type == ObjectByExtruder::Island::Region::INFILL) ? layerm->fills.entities : layerm->perimeters.entities) {
                        ...
                        // Local-Z perimeter clipping: distribute perimeters into per-pass buckets and clip base
                        ...
                        // Grouped manual pattern perimeter splitting
                        ...
                        if (!is_anything_overridden &&
                            ...
                            const unsigned int configured_id = layer_tools.extruder_override != 0 ?
                                layer_tools.extruder_override :
                                region.config().wall_filament.value;
                            if (configured_id != 0 && layer_tools.mixed_mgr->is_mixed(configured_id, layer_tools.num_physical)) {
                                const MixedFilament* mf = layer_tools.mixed_mgr->mixed_filament_from_id(configured_id, layer_tools.num_physical);
                                if (mf != nullptr) {
                                    ...
                                    if (normalized.find(',') != std::string::npos) {
                                        ...
                                        if (split_extrusion_collection_for_multi_perimeter_pattern(
                                                *filtered_extrusions, *layer_tools.mixed_mgr, configured_id,
                                                layer_tools.num_physical, layer_tools.layer_index,
                                                split_by_extruder, bucket_count) && bucket_count >= 2) {
                                            for (size_t extruder_idx = 0; extruder_idx < split_by_extruder.size(); ++extruder_idx) {
                                                std::unique_ptr<ExtrusionEntityCollection>& split_col = split_by_extruder[extruder_idx];
                                                if (!split_col || split_col->entities.empty())
                                                    continue;
                                                const ExtrusionEntityCollection* split_ptr = split_col.get();
                                                local_z_clipped_collections.emplace_back(std::move(split_col));
                        ...
                        if (is_anything_overridden) {
                            entity_overrides = const_cast<LayerTools&>(layer_tools).wiping_extrusions().get_extruder_overrides(filtered_extrusions, layer_to_print.original_object, correct_extruder_id, layer_to_print.object()->instances().size());
                        ...
                        for (unsigned int extruder : printing_extruders)
                        {
                            ...
                            for (size_t i = 0; i <= n_slices; ++ i) {
                                ...
                                if (// extrusions->first_point does not fit inside any slice
                                    ...
                                    if (islands[island_idx].by_region.empty())
                                        islands[island_idx].by_region.assign(print.num_print_regions(), ObjectByExtruder::Island::Region());
                                    islands[island_idx].by_region[region.print_region_id()].append(entity_type, filtered_extrusions, entity_overrides);
                                    break;
                                }
                            }
                        }
    ...
    if (!local_z_pass_refs.empty()) {
            ...
            for (size_t ordered_group_idx = 0; ordered_group_idx < ordered_pass_group.size(); ++ordered_group_idx) {
                ...
                for (unsigned int local_extruder_id : ordered_bucket_extruders) {
                    ...
                    for (InstanceToPrint& instance_to_print : instances_to_print) {
                        const LayerToPrint& ltp = layers[instance_to_print.layer_id];
                        const bool object_layer_over_raft =
                            ltp.object_layer && ltp.object_layer->id() > 0 &&
                            instance_to_print.print_object.slicing_parameters().raft_layers() == ltp.object_layer->id();
        ...
        // Keep wipe tower planning synchronized: restore the pre-pass tool.
